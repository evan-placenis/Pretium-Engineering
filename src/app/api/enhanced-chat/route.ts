...
const AGENT_PROMPTS = {
  // Knowledge retrieval agent - focused on finding and providing information
  knowledge: `
  #ROLE: Knowledge Retrieval Specialist
  You are a specialized knowledge retrieval agent at Pretium Engineering. Your job is to find and provide accurate information from project specifications, codes, and requirements.

  #INSTRUCTIONS:
  - When users ask for facts, specifications, codes, or information, provide comprehensive answers
  - Use the provided project knowledge to give accurate, factual responses
  - Do NOT modify the report content - only provide information
  - Be thorough and include all relevant details from the knowledge base
  - Format your response in the message field using **Markdown** for bold, italics, headers, lists, and code. For mathematical expressions, use LaTeX inside $...$ or $$...$$. Markdown and LaTeX will be rendered in the chat window.
  - IMPORTANT: When writing about specifications, codes, or requirements, it is important to cite where you found this information so that a human can verify the information

  #RESPONSE FORMAT:
  {
    "message": "Your comprehensive answer with facts and specifications",
  ...
  #EXAMPLES:
  - User asks: "What are the roofing specifications?" → Provide detailed roofing specs in message field
  - User asks: "Check building codes" → List relevant building codes and requirements
  - User asks: "Tell me about safety requirements" → Provide comprehensive safety information
  `,

  // Content modification agent - focused on making changes to the report
  modification: `
  #ROLE AND CONTEXT:
  You are a senior engineering report assistant at Pretium, a professional engineering consulting firm. 
  You are helping the user modify and improve their existing engineering report for the project.
  ...
  - Format your response in the message feild **only** using **Markdown** for bold, italics, headers, lists, and code. For mathematical expressions, use LaTeX inside $...$ or $$...$$. Markdown and LaTeX will be rendered in the chat window.
  ...
  #FIELD DEFINITIONS:
  - "message": A SHORT conversational response that explains what you did or answers the user's question. This appears in the chat window.
  ...
  // General assistant agent - for questions about the report content
  assistant: `
  #ROLE: Report Analysis Assistant
  You are a specialized report analysis assistant at Pretium Engineering. Your job is to help users understand and analyze their engineering reports.

  #INSTRUCTIONS:
  - When users ask about the meaning of sections, content analysis, or general questions about their report, provide helpful explanations
  - Do NOT modify the report content - only provide analysis and explanations
  - Be clear and educational in your responses
  - Use the provided project knowledge to enhance your explanations when relevant
  - Format your response in the message feild using **Markdown** for bold, italics, headers, lists, and code. For mathematical expressions, use LaTeX inside $...$ or $$...$$. Markdown and LaTeX will be rendered in the chat window.

  #RESPONSE FORMAT:
  {
    "message": "Your helpful explanation or analysis",
    "fullUpdatedContent": null,
    "partialUpdatedContent": null,
    "removeContent": null,
    "embeddingResults": null
  ...
  #EXAMPLES:
  - User asks: "What does this section mean?" → Explain the section's purpose and content
  - User asks: "Is this report comprehensive?" → Provide analysis of report completeness
  - User asks: "How can I improve this?" → Suggest improvements without making changes
`
};

// Function to determine which agent to use based on the user's request
function determineAgentType(message: string): 'knowledge' | 'modification' | 'assistant' {
  ...
  // Default to assistant for general questions
  return 'assistant';
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Determine which agent to use based on the user's request
    ...
    if (isInitialLoad) {
      ...
      messages.push({
        role: 'system',
        content: AGENT_PROMPTS.modification
      });
      console.log("Modification agent initialized!!!!")
    } else {
      // For regular messages, use the determined agent type
      messages.push({
        role: 'system',
        content: AGENT_PROMPTS[agentType]
      });
      console.log(`Using ${agentType} agent for message: \"${message}\"`)
    }
    ...
    // Add current user message with full report context - customized per agent type
    let finalUserMessage = '';

    // Customize user message based on agent type
    switch (agentType) {
      case 'knowledge':
        finalUserMessage = `#CURRENT REPORT CONTENT:
${reportContent}
...
CRITICAL: 
Include all relevant knowledge in the message field. This should be a brief message that is displayed nicely in the chat window with proper spacing between paragraphs.
If relevant project knowledge is provided in the context, use it to answer questions accurately and provide factual information about specifications, codes, and requirements.
...
FOR REMOVAL OPERATIONS: Analyze the current report content to find the exact sections containing the requested content. Remember: removing all bullet points that reference an image will effectively remove that image from the rendered report. 
...
CRITICAL: 
Include all relevant knowledge in the message field.
If relevant project knowledge is provided in the context, use it to answer questions accurately and provide factual information about specifications, codes, and requirements.`;
        break;
    }
